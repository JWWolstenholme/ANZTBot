...
resultpattern = re.compile('(https://osu.ppy.sh/community/matches/)|(https://osu.ppy.sh/mp/)(?P<id>[0-9]+) (?P<b1>(NM|HD|HR|DT|FM)[0-9]) (?P<b2>(NM|HD|HR|DT|FM)[0-9])')
...
async def handle_match(message):
    ...
    embed.add_field(name=f'{p1} {emotes[finalscore[p1]]}', value=f'Banned {p1b}', inline=True)
    embed.add_field(name=f'{emotes[finalscore[p2]]} {p2}', value=f'Banned {p2b}', inline=True)
    embed.set_footer(text=f'Coded by Diony | Reported by {message.author}')
